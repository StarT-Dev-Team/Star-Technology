            ...
            .aisle('B     B                       ', ' B   B                        ', '  DDD                         ', '  DND                         ', '  DDD                         ', ' B   B                        ', 'B     B                       ') 
                ...
                .where(
                    ...
                    P.kjsBlock('primordial_ware_casing')
                        .or(P.gtBlock('ulv_input_bus').setMaxGlobalLimited(1).setPreviewCount(0))
                        .or(
                            P.ability(PartAbility.COMPUTATION_DATA_RECEPTION)
                                .setExactLimit(1)
                                .setPreviewCount(0)
                        )
                        .or(P.ability(PA.euIn).setMaxGlobalLimited(2).setPreviewCount(0))
                )
                .where('E', P.kjsBlock('aberration_casing'))
                .where('F', P.block('start_core:auxiliary_boosted_fusion_casing_mk2'))
                .where('G', P.kjsBlock('nyanium_engine_intake_casing'))
                .where('H', P.kjsBlock('prismalic_reflector_casing'))
                .where('I', P.kjsBlock('nyanium_pipe_casing'))
                .where('J', P.gtBlock('neutronium_block'))
                .where('K', P.ability(PA.itemOut))
                .where('L', P.kjsBlock('core_casing'))
                .where('@', P.controller(definition))
                .where('N', P.ability(PA.fluidIn))
                .build()
        )
        .workableCasingModel(
